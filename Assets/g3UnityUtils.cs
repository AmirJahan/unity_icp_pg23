...
public class g3UnityUtils
{
    ...
    /// </summary>
    public static DMesh3 UnityMeshToDMesh(Mesh mesh, Transform subTransform = null)
    {


        // I modified this to account foe the initial transformation
        Vector3[] mesh_vertices;

        if (subTransform == null) // this is for target
            mesh_vertices = mesh.vertices;
        else
        {
            // we are adding the 
            Vector3[] mesh_local_vertices = mesh.vertices;
            mesh_vertices = new Vector3[mesh_local_vertices.Length];

            for (int i = 0; i < mesh_local_vertices.Length; ++i)
                mesh_vertices[i] = subTransform.localToWorldMatrix.MultiplyPoint3x4(mesh_local_vertices[i]);
        }
